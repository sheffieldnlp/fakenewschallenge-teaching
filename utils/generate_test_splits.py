...
def split(dataset, base="splits"):
    if not (os.path.exists(base+"/"+"training_ids.txt")
            and os.path.exists(base+"/"+"test_ids.txt")
            and os.path.exists(base+"/"+"dev_ids.txt")):
        if "dev" in os.environ.keys():
            # You REALLY don't need to run this because data set splits are already provided to you.
            generate_split(dataset,base=base)
